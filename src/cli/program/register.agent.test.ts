...
const agentsTemplatesApplyCommandMock = vi.fn();
const agentsTemplatesListCommandMock = vi.fn();
const agentsTemplatesPlanCommandMock = vi.fn();
const agentsTemplatesShowCommandMock = vi.fn();
...
vi.mock("../../commands/agents.js", () => ({
  ...
  agentsTemplatesApplyCommand: agentsTemplatesApplyCommandMock,
  agentsTemplatesListCommand: agentsTemplatesListCommandMock,
  agentsTemplatesPlanCommand: agentsTemplatesPlanCommandMock,
  agentsTemplatesShowCommand: agentsTemplatesShowCommandMock,
...
describe("registerAgentCommands", () => {
  ...
  beforeEach(() => {
    ...
    agentsTemplatesApplyCommandMock.mockResolvedValue(undefined);
    agentsTemplatesListCommandMock.mockResolvedValue(undefined);
    agentsTemplatesPlanCommandMock.mockResolvedValue(undefined);
    agentsTemplatesShowCommandMock.mockResolvedValue(undefined);
  ...
  it("runs agents templates list when root templates command is invoked", async () => {
    await runCli(["agents", "templates"]);
    expect(agentsTemplatesListCommandMock).toHaveBeenCalledWith({}, runtime);
  });

  it("forwards agents builder plan options", async () => {
    await runCli([
      "agents",
      "builder",
      "plan",
      "Create a support bot on Telegram",
      "--template",
      "support-responder",
      "--json",
    ]);
    expect(agentsBuilderPlanCommandMock).toHaveBeenCalledWith(
      {
        brief: "Create a support bot on Telegram",
        template: "support-responder",
        json: true,
      },
      runtime,
    );
  ...
  it("forwards agents builder apply options", async () => {
    await runCli(["agents", "builder", "apply", "Create me a daily digest", "--yes", "--json"]);
    expect(agentsBuilderApplyCommandMock).toHaveBeenCalledWith(
      {
  ...
  it("forwards agents templates list options", async () => {
    await runCli(["agents", "templates", "list", "--json"]);
    expect(agentsTemplatesListCommandMock).toHaveBeenCalledWith(
      {
        json: true,
      },
      runtime,
  ...
  it("forwards agents templates show options", async () => {
    await runCli(["agents", "templates", "show", "daily-briefing", "--json"]);
    expect(agentsTemplatesShowCommandMock).toHaveBeenCalledWith(
      {
  ...
  it("forwards agents templates plan options", async () => {
    await runCli([
      "agents",
      "templates",
      "plan",
    ...
    expect(agentsTemplatesPlanCommandMock).toHaveBeenCalledWith(
      {
        input: "./briefing.yaml",
        set: ["owner_target=@owner"],
        json: true,
      },
      runtime,
    );
  ...
  it("forwards agents templates apply options", async () => {
    await runCli([
      "agents",
      "templates",
      "apply",
    ...
    expect(agentsTemplatesApplyCommandMock).toHaveBeenCalledWith(
      {
