...
`,
  ...
  const templates = agents
    .command("templates")
    .alias("blueprints")
    .description("Inspect starter blueprint bundles, preview plans, and apply blueprints")
    .addHelpText(
      "after",
      () =>
        `
${theme.heading("Examples:")}
${formatHelpExamples([
  ["openclaw agents templates list", "List the built-in starter and stretch templates."],
  ["openclaw agents templates show daily-briefing", "Print the bundled blueprint as YAML."],
  [
    "openclaw agents templates plan daily-briefing",
    "Build a dry-run plan from a starter template.",
  ],
  [
    "openclaw agents templates plan daily-briefing --set owner_target=@me",
    "Resolve template variables while previewing the plan.",
  ],
  [
    "openclaw agents templates plan ./briefing.yaml --json",
    "Build a dry-run plan from a local blueprint file.",
  ],
  [
    "openclaw agents templates apply daily-briefing --set owner_target=@me --yes",
    "Materialize a blueprint into config, workspace files, bindings, and cron jobs.",
  ],
])}
`,
    );
  ...
  templates
    .command("list")
    .description("List bundled agent blueprint templates")
    .option("--json", "Output JSON instead of text", false)
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        await agentsTemplatesListCommand(
          {
            json: Boolean(opts.json),
          },
          defaultRuntime,
  ...
  templates
    ...
    .description("Show a bundled agent blueprint template")
    .option("--json", "Output JSON instead of YAML", false)
    .action(async (templateId, opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        await agentsTemplatesShowCommand(
          {
  ...
  templates
    .command("plan <input>")
    .description("Generate a dry-run plan from a bundled template or local blueprint file")
    .option("--set <key=value>", "Resolve blueprint variables (repeatable)", collectOption, [])
    .option("--json", "Output JSON instead of text", false)
  ...
  templates
    .command("apply <input>")
    .description("Apply a bundled template or local blueprint file")
    .option("--set <key=value>", "Resolve blueprint variables (repeatable)", collectOption, [])
    .option("--yes", "Confirm apply without prompting", false)
    .option("--json", "Output JSON instead of text", false)
    .action(async (input, opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
  ...
  templates.action(async () => {
    await runCommandWithRuntime(defaultRuntime, async () => {
      await agentsTemplatesListCommand({}, defaultRuntime);
    });
  });

  const builder = agents
    .command("builder")
    .description("Turn a plain-English brief into a planned or applied agent blueprint")
    .addHelpText(
      "after",
      () =>
        `
...
${formatHelpExamples([
  [
    'openclaw agents builder plan "Create a bot that summarizes my daily emails every morning at 9am"',
    "Infer the closest starter template and build a plan.",
  ],
  [
    'openclaw agents builder plan "Create a support bot on Telegram" --template support-responder',
    "Force a template when you want to override auto-selection.",
  ],
  [
...
`,
  ...
  builder
    .command("plan <brief>")
    .description("Infer a starter blueprint from a plain-English brief")
  ...
  builder
    .command("apply <brief>")
    .description("Infer and apply a blueprint from a plain-English brief")
    .option("--template <id>", "Force a specific blueprint template")
    .option("--yes", "Confirm apply without prompting", false)
    .option("--json", "Output JSON instead of text", false)
    .action(async (brief, opts) => {
