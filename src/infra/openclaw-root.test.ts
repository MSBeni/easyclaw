...
describe("resolveOpenClawPackageRoot", () => {
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "easyclaw");
    const pkgRoot = path.join(project, "node_modules", "easyclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "easyclaw" }));
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "easyclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "easyclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "easyclaw" }));
  ...
  it("falls through from a non-openclaw moduleUrl candidate to cwd", async () => {
    ...
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "easyclaw" }));
  ...
  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "easyclaw");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "easyclaw.mjs")));
    const pkgRoot = path.join(project, "node_modules", "easyclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "easyclaw" }));
  ...
  it("still resolves legacy openclaw package roots for compatibility", async () => {
    const pkgRoot = fx("legacy-openclaw");
