...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "easyclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "easyclaw.js", entryBasename: "entry.js" },
  { wrapperBasename: "openclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "openclaw.js", entryBasename: "entry.js" },
] as const;

function formatCliErrorPrefix(argv = process.argv): string {
  return `[${resolveCliName(argv)}]`;
}
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        `${formatCliErrorPrefix()} Failed to respawn CLI:`,
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `EasyClaw ${VERSION} (${commit})` : `EasyClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          `${formatCliErrorPrefix()} Failed to resolve version:`,
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          `${formatCliErrorPrefix()} Failed to display help:`,
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`${formatCliErrorPrefix()} ${parsed.error}`);
      process.exit(2);
    }
    ...
    if (!tryHandleRootVersionFastPath(process.argv) && !tryHandleRootHelpFastPath(process.argv)) {
      import("./cli/run-main.js")
        ...
        .catch((error) => {
          console.error(
            `${formatCliErrorPrefix()} Failed to start CLI:`,
            error instanceof Error ? (error.stack ?? error.message) : error,
          );
          process.exitCode = 1;
        });
