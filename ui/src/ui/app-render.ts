...
const lazyBuilder = createLazy(() => import("./views/builder.ts"));
const lazyTemplates = createLazy(() => import("./views/templates.ts"));
  ...
  return html`
    ...
    >
      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        ...
        ${
          state.tab === "templates"
            ? lazyRender(lazyTemplates, (m) =>
                m.renderTemplates({
                  state,
                  onSelectTemplate: (templateId) => {
                    state.templatesSelectedId = templateId;
                    state.templatesPanel = "detail";
                    state.templatesApplyResult = null;
                    state.templatesApplyError = null;
                    state.templatesConfirmApply = false;
                  },
                  onSelectPanel: (panel) => {
                    state.templatesPanel = panel;
                    if (panel === "plan" && state.templatesSelectedId) {
                      m.triggerPlanLoad(state, state.templatesSelectedId);
                    }
                  },
                  onBackToGallery: () => {
                    state.templatesPanel = "gallery";
                    state.templatesSelectedId = null;
                  ...
                  onSetVariable: (key, value) => {
                    state.templatesVariables = { ...state.templatesVariables, [key]: value };
                  },
                  onApply: (templateId) => {
                    m.triggerApply(state, templateId);
                  },
                  onConfirmApply: () => {
        ...
        ${
          state.tab === "nodes"
            ? html`
                ${renderBuilderSetupNotice(state, "nodes")}
        ...
        ${
          state.tab === "logs"
            ? html`
                ${renderBuilderSetupNotice(state, "logs")}
